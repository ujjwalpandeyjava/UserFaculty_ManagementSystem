...
public class RegisterServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (!validateEmailApache) {
				...
				UserDetails userDetails = (UserDetails) sess.get(UserDetails.class, uEmail);
				if (userDetails != null) {
					session = request.getSession();
					session.setAttribute("reg-failed",
					...
					if (uType != null) {
						...
						uDetails = new UserDetails(uFirstName, uLastName, uEmail, uPassword, new Date(), uType);
						...
						session = request.getSession();
						session.setAttribute("reg-sucess", "New '" + uType + "' Registered Sucesfull");
						response.sendRedirect("admin.jsp");
						sendmail(uEmail, uPassword,
								"new ".concat(uType).concat(" - ").concat(uLastName).concat(uLastName));
					} else {
						...
						sendmail(uEmail, uPassword, uFirstName.concat(uLastName));
						response.sendRedirect("register.jsp");
					}
				}
			}
	...
	public void sendmail(String receipent, String password, String name) throws MessagingException {
		...
		Message mess = prepareMessage(sess, myAddress, receipent, password, name);
		Transport.send(mess);
	}

	private static Message prepareMessage(javax.mail.Session sess, String myAddress, String receipent, String pass,
			String name) {
		try {
			...
			String mailBody = "<h1> Welcome " + name + " at Developers' Point </h1>"
					+ "<center> Initial detail of your account with mail id: " + receipent + " are: " + "<br> Name: <b>"
					+ name + "</b>" + "<br> Password: <b>" + pass + "</b>"
